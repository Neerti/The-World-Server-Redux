...
/datum/managed_browser/proc/show_to_user(mob/user)
	if(istype(user, /client))
		switch_client(user)
	else if(istype(user.client, /client))
		switch_client(user.client)


// Swaps the client to another one.
// Required for browsers that are 'shared' between people, e.g. using an ingame console.
// Alternatively, you can have multiple object instances that somehow sync their state together,
// however you will have to do that yourself.
/datum/managed_browser/proc/switch_client(client/new_client)
	my_client = new_client
	display()
